angular.module('app')
    .controller('ProductCtrl',
        ...
        function($scope, appService, appDialog, $timeout) {
            ...
            function initData() {
                appService.ajaxGet('/api/core/product').then(function(d) {
                    $scope.date = d;
                });
                $scope.pageSearch(1);
            };
            ...
            $scope.pageSearch = function(page) {
                var url = '/api/core/product/search?page=' + (page - 1);
            ...
            $scope.remove = function(item) {
                appDialog.confirmDeletion(function(){
                    appService.ajaxDelete('/api/core/product/' + item.id).then(function() {
            ...
            $scope.save = function() {
                $scope.saving = true;
                appService.ajaxSave('/api/core/product', $scope.formModel).then(function(res) {
                    $scope.pageSearch(1);
                    dialog.close();
                }).finally(function() {
                    $scope.saving = false;
