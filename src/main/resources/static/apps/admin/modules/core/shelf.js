angular.module('app')
    .controller('ShelfCtrl',
        ...
        function($scope, appService, appDialog, $timeout) {
            ...
            function initData() {
                appService.ajaxGet('/api/core/shelf').then(function(d) {
                    $scope.date = d;
                });
                $scope.pageSearch(1);
            };
            ...
            $scope.pageSearch = function(page) {
                var url = '/api/core/shelf/search?page=' + (page - 1);
            ...
            $scope.remove = function(item) {
                appDialog.confirmDeletion(function(){
                    appService.ajaxDelete('/api/core/shelf/' + item.id).then(function() {
            ...
            $scope.save = function() {
                $scope.saving = true;
                appService.ajaxPost('/api/core/shelf/saveShelf', $scope.formModel).then(function(res) {
                    $scope.pageSearch(1);
                    dialog.close();
                }).finally(function() {
                    $scope.saving = false;
