angular.module('app')
    .controller('BoxCtrl',
        ...
        function($scope, appService, appDialog, $timeout) {
            ...
            function initData() {
                appService.ajaxGet('/api/core/box').then(function(d) {
                    $scope.date = d;
                });
                $scope.pageSearch(1);
            };
            ...
            $scope.pageSearch = function(page) {
                var url = '/api/core/box/search?page=' + (page - 1);
            ...
            $scope.remove = function(item) {
                appDialog.confirmDeletion(function(){
                    appService.ajaxDelete('/api/core/box/' + item.id).then(function() {
            ...
            $scope.save = function() {
                $scope.saving = true;
                appService.ajaxPost('/api/core/box/saveBox', $scope.formModel).then(function(res) {
                    $scope.pageSearch(1);
                    dialog.close();
                }).finally(function() {
                    $scope.saving = false;
