...
//    }
    ...
    @ApiOperation(value = "Save Area")
    @RequestMapping(value = "/saveArea", method = RequestMethod.POST)
    public Page<Area> saveArea(@RequestBody String param) {
        JsonParser jsonParser = new JsonParser();
        JsonObject jsonObject = jsonParser.parse(param).getAsJsonObject();
        String name = jsonObject.get("name").getAsString();
        String warehouseName = jsonObject.get("warehouseName").getAsString();
        String description = jsonObject.get("description").getAsString();

        return this.areaService.saveArea(name, warehouseName,description);
    }
